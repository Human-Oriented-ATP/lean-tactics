 ...
 */
async function renderHtml(rs, pos, html) {
    if ('text' in html) {
...
export default function HtmlDisplay(props) {
    ...
    const state = useAsyncPersistent(() => renderHtml(rs, props.pos, props.html), [rs, props.pos, props.html])
    if (state.state === 'resolved')
        return state.value
    else if (state.state === 'rejected')
        return React.createElement("span",{className:"red",children:["Error rendering HTML: ", mapRpcError(state.error).message]})
