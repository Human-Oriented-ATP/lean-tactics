...
const InfoDisplayContent = React.memo((props : GoalSelectionProps) => {
    ...
    const goalState = useAsyncPersistent<JSX.Element>(async () => {
      ...
      const html:Html = await rs.call('GoalRendering.rpc', rpcProps);
    ...
    const goal = 
        ...
        : goalState.state === 'loading' ?
            <>Loading...</>
        : goalState.value
    const panelState = useAsyncPersistent<JSX.Element>(async () => {
        const rpcProps:GoalSelectionProps = {
            pos : props.pos, 
            expr : props.expr, 
            locations : selectedLocs.map(goalsLocationToSubexprPos).filter((p : SubexprPos | undefined) : p is SubexprPos => !!p) }
        const html:Html = await rs.call('Panel.rpc', rpcProps);
        return renderHtml(rs, pos, html);
    }, [rs, selectedLocs]);
    const panel =
        panelState.state === 'rejected' ?
            <p color='red'>{mapRpcError(panelState.error).message}</p>
