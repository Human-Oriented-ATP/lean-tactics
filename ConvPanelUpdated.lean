...
def insertEnter (subexprPos : SubExpr.Pos) (goalType : Expr) (cmdStx : Syntax)
  ...
  let inserted := syntaxInsert cmdStx located.pathBeforeConv located.pathAfterConv (toString (subexprPos.toArray.toList)) text
  ...
  return { label := toString (subexprPos.toArray.toList), edit := edit, newCursorPos := inserted.newCursorPos }
...
@[widget_module]
def SelectPanel : Component PanelWidgetProps where
  javascript := "
    ...
    export default function(props) {
      ...
      return e('details', {open: true}, [
        e('summary', {className: 'mv2 pointer'}, 'Select 🔍'),
        e('div', null, inner)
      ])
    }
  "
...
example [Add α] [Neg α] [OfNat α (nat_lit 0)] (f : α → α)
    (h₁ : ∀ (a : α), a + 0 = a)
    (h₂ : ∀ (a b c : α), (a + b) + c = a + (b + c))
    (h₃ : ∀ (a : α), a + (-a) = 0) :
    ∀ (a : α), f a = 0 :=
by with_panel_widgets [SelectPanel]
  intro a
  sorry
