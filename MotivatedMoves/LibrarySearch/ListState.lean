...
protected def append : (xs ys : StateList σ α) → StateList σ α
  | .nil,         bs => bs
...
@[specialize]
protected def foldlM {m : Type u → Type v} [Monad m] {β : Type u} {α σ : Type w} : (f : β → α → σ → m β) → (init : β) → StateList σ α → m β
  | _, b, .nil     => pure b
  | f, b, .cons a s l => do
    let s' ← f b a s
