...
/--
Discrimination tree trie. See `MyDiscrTree`.
-/
...
/-!
Notes regarding term reduction at the `MyDiscrTree` module.
...
-/
structure MyDiscrTree (α : Type) where
  root : PersistentHashMap Key (Trie α) := {}

namespace MyDiscrTree
...
partial def format [ToFormat α] (d : MyDiscrTree α) : Format :=
...
instance [ToFormat α] : ToFormat (MyDiscrTree α) := ⟨format⟩
...
instance : Inhabited (MyDiscrTree α) where
  default := {}
...
/--
  ...
  - We considered ignoring implicit arguments (e.g., `{α : Type}`) since users don't "see" them,
    ...
    ```
    If we ignore the implicit argument `Unit`, the `MyDiscrTree` will say it is a candidate
...
private partial def pushArgsAux (infos : Array Bool) : Nat → Expr → Array Expr → MetaM (Array Expr)
  | i, .app f a, todo => do
    if infos[i]! then
...
/--
  ...
  Different `MyDiscrTree` users may populate this set using, for example, attributes.
...
The parameter at type `MyDiscrTree` controls this value.
See comment at `MyDiscrTree`.
-/
...
private def argsIgnore (fn : Expr) (args : Array Expr) : MetaM (Array Bool) := do
...
private def pushArgs (root : Bool) (todo : Array Expr) (e : Expr) (config : WhnfCoreConfig) : MetaM (Key × Array Expr) := do
  if hasNoindexAnnotation e then
    ...
    let push (k : Key) (nargs : Nat) (todo : Array Expr): MetaM (Key × Array Expr) := do
      let info ← argsIgnore fn e.getAppArgs
      let todo ← pushArgsAux info (nargs-1) e todo
      return (k, todo)
    match fn with
    | .lit v     =>
      return (.lit v, todo)
...
def insertCore [BEq α] (d : MyDiscrTree α) (keys : Array Key) (v : α) (config : WhnfCoreConfig) : MyDiscrTree α :=
...
def insert [BEq α] (d : MyDiscrTree α) (e : Expr) (v : α) (config : WhnfCoreConfig) : MetaM (MyDiscrTree α) := do
    ...
    if isMatch then
      ...
      if ctx.config.isDefEqStuckEx then
        /-
          ...
          The method `MyDiscrTree.getUnify e` returns candidates `c` that may "unify" with `e`.
          That is, `isDefEq c e` may return true. Now, consider `MyDiscrTree.getUnify d (Add ?m)`
...
private def getStarResult (d : MyDiscrTree α) : Array α :=
  ...
  | .node vs cs =>
    if todo.isEmpty then
      ...
      /-
        ...
        A non-dependent arrow may be an instance of a dependent arrow (stored at `MyDiscrTree`). Thus, we also visit the `Key.other` child.
      -/
      | .arrow => visitNonStar .other #[] (← visitNonStar k args result)
      | _      => visitNonStar k args result

private def getMatchRoot (d : MyDiscrTree α) (k : Key) (args : Array Expr) (result : Array α) (config : WhnfCoreConfig) : MetaM (Array α) :=
  match d.root.find? k with
  | none   => return result
  | some c => getMatchLoop args c result config

private def getMatchCore (d : MyDiscrTree α) (e : Expr) (config : WhnfCoreConfig) : MetaM (Key × Array α) :=
  withReducible do
...
-/
def getMatch (d : MyDiscrTree α) (e : Expr) (config : WhnfCoreConfig) : MetaM (Array α) :=
...
partial def getMatchWithExtra (d : MyDiscrTree α) (e : Expr) (config : WhnfCoreConfig) : MetaM (Array (α × Nat)) := do
...
partial def getUnify (d : MyDiscrTree α) (e : Expr) (config : WhnfCoreConfig) : MetaM (Array α) :=
  withReducible do
