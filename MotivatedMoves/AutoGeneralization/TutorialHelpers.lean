...
Retrieving the goal
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -/
elab "getGoalType" : tactic => do logInfo m!"The goal is {← getGoalType}"
example : 2+2=4 := by {getGoalType; trivial}
...
Creating goals
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -/
elab "createGoal" t:term : tactic => do createGoal (← Term.elabTerm t none)
example : True := by {createGoal (2+2=4); trivial; trivial}
...
#term_to_expr (2+3=5)
#eval (q(2+3=5) : Expr) -- using the Qq library
