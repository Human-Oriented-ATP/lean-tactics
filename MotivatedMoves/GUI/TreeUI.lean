...
def Svg.TreeNode.toHtml {f : Svg.Frame} (node : TreeNode f) : Array Html :=
  ...
  let text := Html.element "foreignObject" #[
    ...
    ("width", toJson (charWidth * length)),
    ("height", 30)
  ] #[<p style={json% {"font-family":"Monospace"}}>{.text node.expr.pretty}</p>]
  #[boundary, text]

def frame : Svg.Frame := {
  xmin := 0, ymin := 0, xSize := 10, width := 500, height := 1000
}

def node : Svg.TreeNode frame := {
  point := .px 300 400, expr := .text "qwertyuiop", color := ⟨1.0, 0.6, 0.2⟩, opacity := 0.6
}
