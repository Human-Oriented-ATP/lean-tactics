...
abbrev InteractiveIO := ReaderT RequestContext <| IStateM QueryWidget Json (Exception ⊕ String) IO.RealWorld
...
def queryWidget (q : QueryWidget) : InteractiveIO Json := fun _ => IStateM.askQuestion q
def askWidget (lev : Int) (widget : Html) : InteractiveIO Json := queryWidget (.widget lev widget)

def askUserForm (lev : Int) (form : Html) : InteractiveIO Json := do
...
def askUserInput (lev : Int) (title input : Html) : InteractiveIO String := do
...
def askUserString (lev : Int) (question : Html) : InteractiveIO String :=
  askUserInput lev question <input type="string"/>
def askUserInt (lev : Int) (question : Html) : InteractiveIO Int := do
...
def askUserSelect {α : Type} (lev : Int) (question : Html) (options : List (α × Html))
  ...
  | .ok (answer : Nat) => do
    ...
    return answer
def askUserBool (lev : Int) (question : Html) : InteractiveIO Bool
  := askUserSelect lev question [
    (true, <button>Yes</button>),
    (false, <button>No</button>)
  ]
def askUserConfirm (lev : Int) (message : Html) : InteractiveIO Unit
  := askUserSelect lev message [((), <button>OK</button>)]

def initEdit (pos : Lsp.Position) : InteractiveIO Unit := do
...
def insertLine (line : String) : InteractiveIO Unit := do
...
def runWidget (x : InteractiveIO Unit) : RequestM (QueryWidget × (Json → InteractiveIO Unit)) := fun ctx => do
...
@[server_rpc_method]
def initializeInteraction (code : InteractiveIO Unit) : RequestM (RequestTask (QueryWidget × WithRpcRef JsonToInteractiveMUnit)) :=
  RequestM.asTask do
...
def g (n : Nat) : InteractiveIO Nat := do
  for _ in [:n] do
    if true then
      continue
  return 4
